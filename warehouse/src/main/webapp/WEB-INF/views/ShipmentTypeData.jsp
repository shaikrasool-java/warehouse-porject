...
<nav class="navbar navbar-default">
  <div class="container-fluid">
    ...
    <div class="navbar-header">
     <a class="navbar-brand" href="${pageContext.request.contextPath}/shipmenttype/all">ShipmentType List</a>
    </div>
</div>
...
<form class="navbar-form navbar-left" role="search" action="./searchAllHosp">
  ...
  <a href="${pageContext.request.contextPath}/shipmenttype/reg" class="btn btn-default">Add ShipmentType</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<a href="pdf"><i class="fa fa-file-pdf-o" style="font-size:20px"  title="Export to PdF!">Export to PdF</i></a>
	...
	<table class="table table-striped table-hover">
		...
		<c:forEach items="${shipment}" var="s">
			<tr>
				<td><c:out value=" " /> </td>
				<td><c:out value="${s.id}" /> </td>
				<td><c:out value="${s.mode}" /> </td>
				<td><c:out value="${s.code}" /> </td>
				<td><c:out value="${s.enabled}" /> </td>
				<td><c:out value="${s.grade}" /> </td>
				<td><c:out value="${s.dsc}" /></td>
				<td><a href="edit?id=${s.id}"class="btn btn-primary" role="button"data-toggle="tooltip" title="Edit!"><i class="fa fa-edit">  Edit</a></td>
				<td><a href="delete?id=${s.id}"class="btn btn-danger" role="button" onclick="myFunction()" data-toggle="tooltip" title="Delete!"><i class="fa fa-trash"/>  Delete</a></td>
			</tr>
		</c:forEach>
	</table>
	</div>
